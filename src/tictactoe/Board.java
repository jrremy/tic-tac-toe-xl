...
public class Board implements ActionListener {
      ...
      for (int r = 0; r < rows; r++) {
         for (int c = 0; c < cols; c++) {
            ...
            if (r == rows - 1 && c == cols - 1) {
               // The size of the boardFrame is determined by the position of the bottom right
               // tile
               frameWidth = x + 105;
               frameHeight = y + 130;
            }
   ...
   /**
    * Performs an opponent move for the singleplayer mode. The computer opponent
    * places an O on a random tile that has not been used yet.
    */
    ...
    * @return "win" if the turn is a win, "tie" if the turn is the last on the
    *         board and isn't a win, "none" otherwise.
    */
   private String check(String letter, int r, int c) {
      // Centered vertical check.
      if (r > 0 && r < rows - 1) {
         if (tiles[r][c].getText().equals(letter) && tiles[r - 1][c].getText().equals(letter)
               && tiles[r + 1][c].getText().equals(letter)) {
            return "win";
         }
      }
      ...
      if (c > 0 && c < cols - 1) {
         if (tiles[r][c].getText().equals(letter) && tiles[r][c - 1].getText().equals(letter)
               && tiles[r][c + 1].getText().equals(letter)) {
            return "win";
         }
      }
      ...
      if ((r > 0 && r < rows - 1) && (c > 0 && c < cols - 1)) {
         if (tiles[r][c].getText().equals(letter) && tiles[r - 1][c - 1].getText().equals(letter)
               && tiles[r + 1][c + 1].getText().equals(letter)) {
            return "win";
         }
      }
      ...
      if (r < rows - 2) {
         if (tiles[r][c].getText().equals(letter) && tiles[r + 1][c].getText().equals(letter)
               && tiles[r + 2][c].getText().equals(letter)) {
            return "win";
         }
      }
      ...
      if (c < cols - 2) {
         if (tiles[r][c].getText().equals(letter) && tiles[r][c + 1].getText().equals(letter)
               && tiles[r][c + 2].getText().equals(letter)) {
            return "win";
         }
      }
      ...
      if (r < rows - 2 && c < cols - 2) {
         if (tiles[r][c].getText().equals(letter) && tiles[r + 1][c + 1].getText().equals(letter)
               && tiles[r + 2][c + 2].getText().equals(letter)) {
            return "win";
         }
      }
      ...
      // If all tiles are used, meaning this turn used the last available tile, then
      // it's a tie.
   ...
   /**
    * Edits the board to show the result of the game and notifies via a
    * JOptionPane.
    * 
    * @param letter The result of the game; "X" for player X win, "O" for player O
    *               win, "T" for tie.
    */
   private void winScreen(String letter, int mode) {
      ...
      if (letter.equals("X")) {
         ...
         int winMsg;
         if (mode == 0) {
            winMsg = JOptionPane.showOptionDialog(null, "You win!", "You win!", JOptionPane.PLAIN_MESSAGE, 1, thumbsUp,
                  buttons, buttons[1]);
         } else {
            winMsg = JOptionPane.showOptionDialog(null, "Player X Wins!", "Player X Wins!", JOptionPane.PLAIN_MESSAGE,
                  1, xWin, buttons, buttons[1]);
         }
         if (winMsg == 0) {
            System.exit(0);
         } else if (winMsg == 1) {
      ...
      // If singleplayer loss or player O win in multiplayer, display win with red
      // board.
         ...
         int winMsg;
         if (mode == 0) {
            winMsg = JOptionPane.showOptionDialog(null, "You lose.", "You lose.", JOptionPane.PLAIN_MESSAGE, 1,
                  thumbsDown, buttons, buttons[1]);
         } else {
            winMsg = JOptionPane.showOptionDialog(null, "Player O Wins!", "Player O Wins!", JOptionPane.PLAIN_MESSAGE,
                  1, oWin, buttons, buttons[1]);
         }
         if (winMsg == 0) {
            System.exit(0);
         } else if (winMsg == 1) {
      ...
      // If neither player won, display tie with gray board.
         ...
         String[] buttons = { "Quit", "Play again" };
         int winMsg = JOptionPane.showOptionDialog(null, "It's a tie.", "You lose.", JOptionPane.PLAIN_MESSAGE, 1, null,
               buttons, buttons[1]);
         if (winMsg == 0) {
            System.exit(0);
         } else if (winMsg == 1) {
      ...
      for (int r = 0; r < rows; r++) {
         for (int c = 0; c < cols; c++) {
            ...
            if ((e.getSource() == tiles[r][c].getButton()) && !tiles[r][c].getUsed()) {
               ...
               // Tile button's text is set to the turn field because of the alternating turns
               // in multiplayer.
               // In singleplayer and by default, turn is always "X".
               tiles[r][c].getButton().setText(turn);
               tiles[r][c].setUsed(true);

               // If the mode is singleplayer, a win or tie is displayed if the turn is a win
               // or tie, or a random opponent turn is performed.
               ...
               // Otherwise, if the mode is multiplayer, a win or tie is displayed if the turn
               // is a win or tie, or it becomes the other player's turn.
               else {
